...
public class HoloCircleSeekBar extends View {
    ...
    private String wheel_color_attr, wheel_unactive_color_attr,
            pointer_color_attr, pointer_halo_color_attr, text_color_attr, /*ROBERT*/ text_unit, font_name, font_style;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        if(font_name!=null) {
            Typeface myTypeface = Typeface.createFromAsset(getContext().getAssets(), "fonts/"+font_name);
            textPaint.setTypeface(myTypeface);
        }

        textUnitPaint = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.LINEAR_TEXT_FLAG);
        textUnitPaint.setColor(text_color);
        textUnitPaint.setStyle(Style.FILL_AND_STROKE);
        textUnitPaint.setTextAlign(Align.LEFT);
        // canvas.drawPaint(textUnitPaint);
        textUnitPaint.setTextSize(text_size/2);
        if(font_name!=null) {
            Typeface myTypeface = Typeface.createFromAsset(getContext().getAssets(), "fonts/"+font_name);
            textUnitPaint.setTypeface(myTypeface);
        }
    ...
    private void initAttributes(TypedArray a) {
        ...
        text_unit = a.getString(R.styleable.HoloCircleSeekBar_text_unit);
        ...
        font_name = a.getString(R.styleable.HoloCircleSeekBar_font_name);
        font_style = a.getString(R.styleable.HoloCircleSeekBar_font_style);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        textUnitPaint.getTextBounds(text_unit, 0, text_unit.length(), bounds);
        ...
        if (show_text) {
            canvas.drawText(
                ...
                canvas.drawText(
                        text_unit,
                        (mColorWheelRectangle.centerX())
                                - (textUnitPaint.measureText(text_unit) / 2),
                        (mColorWheelRectangle.centerY() + bounds.height() / 2) + (bounds.height() + (bounds.height()/2)),
                        textUnitPaint);
    ...
    public void setInitPositionCountdown(int position) {
        if(position == max) {
            arc_finish_radians = end_wheel - 1;
            ...
            mAngle = calculateAngleFromRadians(arc_finish_radians > end_wheel ? end_wheel
                    : arc_finish_radians);
            text = String.valueOf(max-position);
        }

        invalidate();
    }
    public void setMaxPosition(int max_position) {
        max = max_position;
    }
